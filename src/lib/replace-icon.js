...
export function replaceIconInRow(itemRow, provider, iconPack) {
   ...
   const fileName = itemRow
      ...
      .trim()
   if (!fileName)
      return // fileName couldn't be found or we don't have a match for it.

   // SVG to be replaced.
   const iconEl = itemRow.querySelector(provider.selectors.icon)
   if (iconEl?.getAttribute('data-materialiconic'))
      return
   if (iconEl)
...
function replaceIcon(iconEl, fileName, itemRow, provider, iconPack) {
   ...
   if (fileName.length <= 255) {
      for (let i = 0; i < fileName.length; i += 1) {
         if (fileName[i] === '.')
            fileExtensions.push(fileName.slice(i + 1))
      }
   }
...
export function replaceElementWithIcon(iconEl, iconName, fileName, iconPack, provider) {
   ...
   newSVG.setAttribute('data-materialiconic', 'icon')
   newSVG.setAttribute('data-materialiconic-iconname', iconName)
   newSVG.setAttribute('data-mmaterialiconic-filename', fileName)
...
function lookForIconPackMatch(iconPack, lowerFileName) {
   ...
   switch (iconPack) {
            case 'angular':
               ...
               break
            case 'angular_ngrx':
               if (iconsList[`folder-ngrx-${lowerFileName}.svg`])
