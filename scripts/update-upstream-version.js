...
function getUpstreamVersion() {
   return fetch('https://raw.githubusercontent.com/nyxblabs/materialiconic/main/package.json')
      .then(res => res.json())
...
function getUpstreamCommit() {
   return octokit
      .request('GET /repos/nyxblabs/materialiconic/commits', { per_page: 1 })
      .then(res => res.data?.[0].sha)
}

function getLastUpstreamVersion() {
   return fs.readFile(upstreamVersionFilePath, { encoding: 'utf8' }).then(data => data.trim())
}

async function updateReadmeBadge(version) {
